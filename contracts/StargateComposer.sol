    ...
    ReentrancyGuard
{
    ...
    function instantRedeemLocal(
        ...
        amountSD = stargateRouter.instantRedeemLocal(
            _srcPoolId,
            _amountLP,
            _to
        );
    ...
    function sendCredits(
        ...
        stargateRouter.sendCredits{value: msg.value}(
            _dstChainId,
            _srcPoolId,
            _dstPoolId,
            _refundAddress
    ...
    function quoteLayerZeroFee(
        ...
        return
            stargateBridge.quoteLayerZeroFee(
                _chainId,
                _functionType,
                peer,
                newPayload,
    ...
    function swapETHAndCall(
        ...
        {
            require(
                msg.value > _swapAmount.amountLD,
                "Stargate: msg.value must be > _swapAmount.amountLD"
            );
            require(
                stargateEthVaults[wethPoolId] != address(0),
                "Stargate: Pool does not exist"
            );
            IStargateEthVault(stargateEthVaults[wethPoolId]).deposit{
                value: _swapAmount.amountLD
            }();
            IStargateEthVault(stargateEthVaults[wethPoolId]).approve(
                address(stargateRouter),
    ...
    function sgReceive(
        ...
        (bool success, bytes memory data) = _token.call(
            abi.encodeWithSelector(SELECTOR, intendedReceiver, _amountLD)
        );
        if (success && (data.length == 0 || abi.decode(data, (bool)))) {
            ...
            (bool safeCallSuccess, bytes memory reason) = intendedReceiver
                .safeCall(externalGas, 0, 150, callData); // only return 150 bytes of data

            if (!safeCallSuccess) {
                payloadHashes[_srcChainId][_srcAddress][_nonce] = keccak256(
                    abi.encodePacked(intendedReceiver, callData)
                );
            ...
            emit ComposedTokenTransferFailed(
                _token,
                intendedReceiver,
                _amountLD
            );
    ...
    function clearCachedSwap(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(_receiver, _sgReceiveCallData)
        );
        require(
            payloadHashes[_srcChainId][_srcAddress][_nonce] == hash,
            "Stargate: invalid hash"
        );
        delete payloadHashes[_srcChainId][_srcAddress][_nonce];

        (bool success, bytes memory reason) = _receiver.safeCall(
            gasleft(),
            0,
            150,
            _sgReceiveCallData
    ...
    function setTransferOverhead(uint256 _transferOverhead) external onlyOwner {
        transferOverhead = _transferOverhead;
    }

    function setStargateEthVaults(
        uint256 _poolId,
        address _stargateEthVault
    ) public onlyOwner {
    ...
    function setWethPoolId(uint256 _wethPoolId) external onlyOwner {
        wethPoolId = _wethPoolId;
    }

    function setPeer(uint16 _chainId, address _peer) external onlyOwner {
    ...
    function getPoolInfo(
        uint256 _poolId
    ) external returns (PoolInfo memory poolInfo) {
        return _getPoolInfo(_poolId);
    }

    function _getPoolInfo(
        uint256 _poolId
    ) internal returns (PoolInfo memory poolInfo) {
